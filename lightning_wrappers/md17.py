...
class PONITA_MD17(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        self.test_metrics_energy = nn.ModuleList(
            [torchmetrics.MeanAbsoluteError() for r in range(self.repeats)]
        )
        self.test_metrics_force = nn.ModuleList(
            [torchmetrics.MeanAbsoluteError() for r in range(self.repeats)]
        )
    ...
    def set_dataset_statistics(self, dataset):
        ...
        print(
            "Min-max range of distances between atoms in the dataset:",
            self.min_dist,
            "-",
            self.max_dist,
        )
    ...
    def training_step(self, graph):
        ...
        force_loss = (
            torch.mean(torch.sum((pred_force - graph.force / self.scale) ** 2, -1))
            / 3.0
        )
    ...
    def test_step(self, graph, batch_idx):
        ...
        # Compute results
        pred_energy_repeated, pred_force_repeated = self.pred_energy_and_force(
            graph_repeated
        )
        # Unrotate results
        rot_T = rot.transpose(-2, -1)
        pred_force_repeated = self.rotation_transform.rotate_attr(
            pred_force_repeated, rot_T
        )
        # Unwrap predictions
        pred_energy_repeated = pred_energy_repeated.unflatten(
            0, (self.repeats, batch_size)
        )
        pred_force_repeated = pred_force_repeated.unflatten(
            0, (self.repeats, batch_length)
        )
        # Compute the averages
        for r in range(self.repeats):
            pred_energy, pred_force = (
                pred_energy_repeated[: r + 1].mean(0),
                pred_force_repeated[: r + 1].mean(0),
            )
            self.test_metrics_energy[r](
                pred_energy * self.scale + self.shift, graph.energy
            )
