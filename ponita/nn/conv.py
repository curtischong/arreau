...
class Conv(torch_geometric.nn.MessagePassing):
    ...
    def __init__(
        self, in_channels, out_channels, attr_dim, bias=True, aggr="add", groups=1
    ):
        ...
        self.kernel = torch.nn.Linear(
            attr_dim, int(in_channels * out_channels / groups), bias=False
        )
        if bias:
...
class FiberBundleConv(torch_geometric.nn.MessagePassing):
    ...
    def __init__(
        ...
        if self.separable:
            ...
            self.fiber_kernel = torch.nn.Linear(
                attr_dim, int(in_channels * out_channels / groups), bias=False
            )
        else:
            self.kernel = torch.nn.Linear(
    ...
    def forward(self, x, edge_index, edge_attr, fiber_attr=None, **kwargs):
        ...
        if self.separable:
            ...
            if self.depthwise:
                x_2 = (
                    torch.einsum("boc,opc->bpc", x_1, fiber_kernel)
