        ...
        for mi, di in self.f_in.structure:
            for mo, do in self.f_out.structure:
                self.kernel_unary[f"({di},{do})"] = PairwiseConv(
                    di, mi, do, mo, edge_dim=edge_dim, act_fn=act_fn
                )
...
class RadialFunc(nn.Module):
    ...
    def __init__(self, num_freq, in_dim, out_dim, edge_dim: int = 0, act_fn=nn.ReLU()):
        """NN parameterized radial profile function.
...
class PairwiseConv(nn.Module):
    ...
    def __init__(
        ...
        self.rp = RadialFunc(self.num_freq, nc_in, nc_out, self.edge_dim, act_fn=act_fn)
...
class GNormSE3(nn.Module):
    ...
    def __init__(self, fiber, act_fn=nn.ReLU(inplace=True), num_layers: int = 0):
        """Initializer.

        Args:
            fiber: Fiber() of feature multiplicities and types
            act_fn: nonlinearity to use everywhere
    ...
    def __repr__(self):
        return f"GNormSE3(num_layers={self.num_layers}, nonlin={self.act_fn})"
...
class GConvSE3Partial(nn.Module):
    ...
    def __init__(self, f_in, f_out, edge_dim: int = 0, act_fn=nn.ReLU()):
...
class GSE3Res(nn.Module):
    ...
    def __init__(
        ...
        self.GMAB["v"] = GConvSE3Partial(
            f_in, self.f_mid_out, edge_dim=edge_dim, act_fn=act_fn
        )
        self.GMAB["k"] = GConvSE3Partial(
            f_in, self.f_mid_in, edge_dim=edge_dim, act_fn=act_fn
        )
        ...
        # Skip connections
        # TODO: skip connection is currently commented out
